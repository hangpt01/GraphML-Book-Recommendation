...
class HCCF(GraphRecommender):
    ...
    def calcLosses(self, ancs_idx, poss_idx, ancs, poss, negs, gcnEmbedsLst, hyperEmbedsLst, reg):
        bprLoss = bpr_loss(ancs, poss, negs)
        sslLoss = 0
        for i in range(self.nLayers):
            embeds1 = gcnEmbedsLst[i].detach()
            embeds2 = hyperEmbedsLst[i]
            sslLoss += contrastLoss(embeds1[:self.data.n_users], embeds2[:self.data.n_users], torch.unique(ancs_idx.long()), self.temp)\
                         + contrastLoss(embeds1[self.data.n_users:], embeds2[self.data.n_users:], torch.unique(poss_idx.long()), self.temp)
    ...
    def train(self, load_pretrained=False):
        ...
        for ep in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batchSize)):
                ...
                loss_rec, loss_ssl = self.calcLosses(user_idx, pos_idx, anchor_emb, pos_emb, neg_emb, gcnEmbedsLst, hyperEmbedsLst, self.reg)
