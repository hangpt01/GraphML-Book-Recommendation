...
def _create_train_test_file_missing(dir, pct_missing):
...
def generate_noisy_data(dir, user_neg_dict, pct):
    ...
    df_noise = pd.DataFrame(data_train_noise, columns=['user', 'item', 'score'])
    def select_noise(row):
        item_noise = random.choice(user_neg_dict[row['user']])
        return item_noise
    df_noise['item'] = df_noise.apply(select_noise, axis=1)
    data_train_noise_final = df_noise[['user', 'item', 'score']].to_numpy()
...
if __name__ == '__main__':
    ...
    #     _create_train_test_file_missing(dir, pct)
